...
для elastic search на каждую ноду рекомендуется соотношение 2 ядра : 16 GB RAM, также из соотношения ```ram:rom=1:16``` и учитывая, что общий объём данных используемый для поиска составляет(по комментариям поиск не производится) ~ 1.7 TB, если на каждой ноде 4 ядра, 32 Gb RAM и 512 Gb, понадобится > 4 нод
...
| Auth           | Yandex.Cloud | 6430/8GB/1xNVMe64Gb/10Gb/s     | 14  | 2   | $8 000  | $120               |
