...
для elastic search на каждую ноду рекомендуется соотношение 2 ядра : 16 GB RAM, также из соотношения ```ram:rom=1:16``` и учитывая, что общий объём данных используемый для поиска составляет(по комментариям поиск не производится) ~ 1.7 TB + для кода 6300 TB, из [рекомендаций по конфигурации](https://octoperf.com/blog/2018/09/21/optimizing-elasticsearch/#network)
- Узлы данных (Data Nodes):

Количество узлов: 50 узлов.
Хранилище: 140 TB на каждый узел.
...
- Клиентские узлы (coordinating node):
...
Шардирование: 100 шардов (2 шарда на узел данных).
Репликация: 1 реплика на каждый шард.
...
Размер данных: Users + Repos ~ 110 Gb. Возьмём для расчёта 5 шардов, получаем ~ 22 Gb на шард, с учётом индексов и кэширования запросов, на instance получим:
...
Размер данных: Pull_request_comments + Pull_request ~  1.7 TB. Аналогично с репликами в предыдущем случае на instance получаем:
