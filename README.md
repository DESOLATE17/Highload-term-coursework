...
рестарт. Так как все сервисы - stateless поды, то перезапуск и реплицирование обезопасят от даунтайма.

Для БД - спользование реплицирования и шардирования обеспечивают отказоустойчивость при многочисленных транзакциях на
чтение / запись, также необходимы езервное копирование и снапшоты для возможности восстановления.
...
Так как через сервис авторизации проходит множество запросов, его развернем на более мощеных серверах.
