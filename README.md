...
Для балансировки трафика внутри датацентра будут использоваться **L7 балансировщики** - **Envoy**, так как он удобно кастомизируется и имеет хорошую производительность, а статику будет разадавать CDN, плюс есть возможность динамической конфигурации и мониторинга, кроме того envoy позволяет осуществить service mesh для балансировки между контейнерами(например, Istio).
...
```postgresql
    8(bigint) + 44(name) + 54(email) + 64(password) + 104(avatar_url) = 274 байта
...
Для поиска по коду, коммитам, пул реквестам и репозиториям можно использовать **Elasticsearch** - мощный и гибкий поисковый и аналитический движок, предназначенный для обработки и анализа больших объемов данных. Elasticsearch позволяет GitHub быстро и эффективно осуществлять поиск, фильтрацию и анализ данных.
