...
```postgresql
    ...
    655 * 2100M(420М репозиториев, в среднем 5 issue в репозитории) = 1,375 Тб
```
...
Для поиска по
коду([результаты поиска с учётом дефолтной бранчи репозиториев](https://docs.github.com/en/search-github/github-code-search/about-github-code-search),
коммитам, пул реквестам и репозиториям можно использовать **Elasticsearch** - мощный и гибкий
...
рестарт, также автоскейлинг кубера позволит обеспечить практически мгновенное горизонтальное масштабирование. Так как
все сервисы - stateless поды, то перезапуск и реплицирование обезопасят от даунтайма.
...
Большая часть запросов идет с проверкой наличия токена в redis, примерно 5-10% на login/signUp, где в качестве
hash-функции
будет использован bcrypt(при тестировании запроса с использованием bcrypt+походом в redis получилось 130rps на 8 cpu и
рекомендовано 1 cpu:512 Mb ram). При соотношении
5% login/signUp, 95% - поход в redis для проверки, получим следующие значения:

| Тип запроса  | RPS   | CPU | RAM   |
|--------------|-------|-----|-------|
| login/signUp | 715   | 44  | 22 Gb |
| checkAuth    | 13585 | 13  | 3 Gb  |

**Search**

Для поиска передаётся около 2 MB метаданных, учитывая количество rps и необходимые коннекты к elastic-у и postgres,
получим около 20 Gb
...
|---------------|-------------------------------|-----|-------|-------------|
| Auth          | 14300                         | 57  | 25 Gb | 1.16 Gbit/s |
| Search        | 463                           | 8   | 20 Gb | 1 Gbit/s    |
| Repo          | 3750                          | 3   | 1 Gb  | 0.5 Gbit/s  |
| Issue         | 1404                          | 4   | 5 Gb  | 0.1 Gbit/s  |
| Pull requests | 1984                          | 4   | 5 Gb  | 0.15 Gbit/s |

**Elastic Search**

Согласно [рекомендациям](https://opster.com/guides/elasticsearch/capacity-planning/elasticsearch-hardware-requirements/#:~:text=As%20a%20general%20guideline%2C%20it,or%20more%20RAM%20per%20node.)
для elastic search на каждую data ноду рекомендуется соотношение 2 ядра : 16 GB RAM, также
для [elastic search принято](https://habr.com/ru/companies/galssoftware/articles/470640/):
```Для определения объёма памяти нод для хранения, Elastic рекомендует пользоваться следующей логикой: «горячие» → 1:30 (30Гб дискового пространства на каждый гигабайт памяти), «тёплые» → 1:100, «холодные» → 1:500)```
Имена репозиториев, pull requests, issues и код наиболее популярных репозиториев можно отнести к первичным данным для
поиска и хранить как "горячие", а код остальных репозиториев как "тёплые", также гитхаб осуществляет поиск только
по [дефолтной ветке репозитория](https://docs.github.com/en/search-github/github-code-search/about-github-code-search) и
соответственно индексирует файлы только в ней,
то рассчитывая объём данных для хранения в elastic-е (по комментариям поиск не производится) ~ 1.7 TB + весь код 6300
TB, а учитывая что в качестве ветки для поиска берётся только main/master и elastic сжимает данные на 20-30%, возьмём
примерно 2500 TB.
Возьмём с этим условием в качестве "горячих" данных 30% , то есть ~750 TB, тогда "тёплых" ~ 1750 TB

- Узлы данных (Data Nodes):

"Горячие":
Количество узлов: 100 узлов.
Хранилище: 7,5 TB на каждый узел.
Процессор: 32 ядра (два процессора по 16 ядер).
Оперативная память: 256 GB RAM.

"Тёплые":
Количество узлов: 70 узлов.
Хранилище: 25 TB на каждый узел.
Процессор: 32 ядра (два процессора по 16 ядер).
Оперативная память: 256 GB RAM.

- Мастер-узлы (Master Nodes):

Количество узлов: 3 узла.
Процессор: 16 ядер (один процессор).
Оперативная память: 64 GB RAM.

- Клиентские узлы (Coordinating nodes):

Количество узлов: 5 узлов.
Процессор: 16 ядер (один процессор).
Оперативная память: 64 GB RAM.

### Postgres

Для размещения баз рекомендован - bare-metal. Min количество реплик 2 (master, slave, slave)

#### Repos

Размер данных: Users + Repos ~ 110 Gb. Возьмём для расчёта 5 шардов, получаем ~ 22 Gb на шард, с учётом индексов и
кэширования запросов, на instance получим:

| RAM   | CPU      | Storage   |
|-------|----------|-----------|
| 32 Gb | 32 cores | NVMe128Gb |

#### PRs

Размер данных: Pull_request_comments + Pull_request ~  1.7 TB. Аналогично с репликами в предыдущем случае на instance
получаем:

| RAM   | CPU      | Storage |
|-------|----------|---------|
| 96 Gb | 64 cores | NVMe2TB |

#### Issues

Размер данных: Issues_comments + Issues ~ 2 TB. Аналогично с репликами в предыдущем случае на instance получаем:

| RAM   | CPU      | Storage |
|-------|----------|---------|
| 96 Gb | 64 cores | NVMe2TB |

Используя конфигуратор Yandex.Cloud:

| Сервис                     | Хостинг      | Конфигурация                   | Cpu | Cnt | Покупка | Аренда/Амортизация |
|----------------------------|--------------|--------------------------------|-----|-----|---------|--------------------|
...
| Elastic hot data nodes     | Yandex.Cloud | 6430/256GB/2xSSD3840Gb/1Gb/s   | 32  | 100 | -       | $1600              |
| Elastic warm data nodes    | Yandex.Cloud | 6430/256GB/8xHDD3200Gb/1Gb/s   | 32  | 70  | -       | $1500              |
| Elastic master nodes       | Yandex.Cloud | 6430/64GB/1xSSD32Gb/10Gb/s     | 16  | 3   | -       | $180               |
| Elastic coordinating nodes | Yandex.Cloud | 6430/64GB/1xSSD32Gb/10Gb/s     | 16  | 5   | -       | $180               |
| Postgres Repos             | Yandex.Cloud | 6430/32GB/1xNVMe128Gb/10Gb/s   | 32  | 15  | -       | $320               |
| Postgres PRs               | Yandex.Cloud | 6430/96GB/1xNVMe2TB/10Gb/s     | 64  | 15  | -       | $850               |
| Postgres Issues            | Yandex.Cloud | 6430/96GB/1xNVMe2TB/10Gb/s     | 64  | 15  | -       | $850               |
